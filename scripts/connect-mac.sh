...
if command -v wg >/dev/null 2>&1; then
  ...
  if command -v brew >/dev/null 2>&1; then
    log "  Installing wireguard-tools via Homebrew..."
    brew install wireguard-tools
  else
    err "Homebrew not found. Install WireGuard manually:"
    err "  brew install wireguard-tools"
    err "  — or install the WireGuard app from the Mac App Store"
    exit 1
  fi
fi
