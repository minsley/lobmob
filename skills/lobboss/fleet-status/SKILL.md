...
This shows WireGuard peers, active lobster droplets, and open PRs.
...
doctl compute droplet list --tag-name "$LOBSTER_TAG" --format ID,Name,PublicIPv4,Status,Memory,VCPUs,Created --no-header
```
...
### 5. Cost estimate
Count active lobster-hours and estimate current burn rate based on droplet sizes.
...
- Add new lobsters when spawned
- Mark unreachable lobsters
- Remove destroyed lobsters
